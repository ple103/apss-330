	...
	public class LoggerCategory{
		...
		public LoggerCategory(IntPtr obj){
			...
			while (nLevelsIndex < nLevelsSize) {
				csLevels.Add(Marshal.PtrToStringAnsi(Marshal.ReadIntPtr(nLevels + (nLevelsIndex * Marshal.SizeOf(nLevels)))));
				nLevelsIndex++;
			}

			description = Marshal.PtrToStringAnsi(VidyoLoggerCategoryGetdescriptionNative(objPtr));
			levels = csLevels;
			name = Marshal.PtrToStringAnsi(VidyoLoggerCategoryGetnameNative(objPtr));
